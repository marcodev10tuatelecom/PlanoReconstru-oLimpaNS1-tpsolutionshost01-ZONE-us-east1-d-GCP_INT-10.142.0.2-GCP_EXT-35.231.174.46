#!/usr/bin/env bash
# ============================================================
# TPS-NS1-CLEANROOM-MASTER-FOUNDATION v2.2.0
...
for c in curl python3 systemctl install sha256sum ffmpeg ffprobe runuser dig nginx named-checkconf find pgrep getent usermod; do
...
ffprobe -v error -read_intervals '%+3' -show_entries stream=codec_type -of default=nw=1:nk=1 \
  http://127.0.0.1:18888/radiopop/index.m3u8 > "$WORK/lab-ffprobe.txt" 2> "$WORK/lab-ffprobe.err" || fatal 'LAB_HLS_DECODE_FAIL'
...
log 'RESULT=PASS_FOUNDATION_R00_R07_V2_2'
